#!/usr/bin/env python3
"""
Tradutor MIDI Aprimorado para MicroDrum - Corrige mensagens MIDI malformadas
"""
...
class MidiTranslatorThread(QThread):
    ...
    def __init__(self, serial_port, baud_rate, midi_port, note_map):
        super().__init__()
    ...
    def process_buffer(self, midi_out):
        ...
        while i < len(self.buffer):
            ...
            if self.buffer[i] == 0xC0:  # Program Change (pode ser um marcador)
                ...
                if i + 2 < len(self.buffer):
                    # Extrai os dados
                    data1 = self.buffer[i+1]  # Nota original
                    data2 = self.buffer[i+2]  # Velocidade
                    
                    # Mapeia a nota se estiver no mapeamento
                    mapped_note = self.note_map.get(data1, data1)
                    
                    # Ajusta a velocidade se for muito baixa
                    velocity = max(data2, 1) if data2 > 0 else 64  # Velocidade mínima de 1, ou 64 se for 0
                    
                    # Cria uma mensagem Note On no canal 10 (bateria)
                    midi_msg = [0x99, mapped_note, velocity]  # 0x99 = Note On canal 10
                    
                    # Envia a mensagem MIDI
                    midi_out.send_message(midi_msg)
                    self.midi_message.emit(midi_msg)
                    self.log_message.emit(f"Nota traduzida: {data1}->{mapped_note}, velocidade: {data2}->{velocity}")
...
class MidiTranslatorWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.setWindowTitle("Tradutor MIDI Aprimorado para MicroDrum")
        ...
        # Área de mapeamento de notas
        mapping_layout = QVBoxLayout()
        mapping_label = QLabel("Mapeamento de Notas:")
        mapping_label.setAlignment(Qt.AlignCenter)
        mapping_layout.addWidget(mapping_label)
        
        # Grid para mapeamento de notas
        grid_layout = QGridLayout()
        
        # Cabeçalhos
        grid_layout.addWidget(QLabel("Pad"), 0, 0)
        grid_layout.addWidget(QLabel("Nota Original"), 0, 1)
        grid_layout.addWidget(QLabel("Nota Mapeada"), 0, 2)
        grid_layout.addWidget(QLabel("Instrumento"), 0, 3)
        ...
        row = 1
        for orig_note, mapped_note in self.note_map.items():
            grid_layout.addWidget(QLabel(f"Pad {orig_note}"), row, 0)
            grid_layout.addWidget(QLabel(str(orig_note)), row, 1)
            
            spinbox = QSpinBox()
            spinbox.setMinimum(0)
            spinbox.setMaximum(127)
            spinbox.setValue(mapped_note)
            spinbox.valueChanged.connect(lambda value, note=orig_note: self.update_note_map(note, value))
            grid_layout.addWidget(spinbox, row, 2)
            self.note_spinboxes[orig_note] = spinbox
            
            instrument = instruments.get(mapped_note, "Desconhecido")
            grid_layout.addWidget(QLabel(instrument), row, 3)
            
            row += 1
            
        mapping_layout.addLayout(grid_layout)
    ...
    def update_note_map(self, orig_note, new_value):
        """Atualiza o mapeamento de notas"""
        self.note_map[orig_note] = new_value
        self.log_message(f"Mapeamento atualizado: Pad {orig_note} -> Nota {new_value}")
    ...
    def start_translator(self):
        ...
        self.log_message("Mapeamento de notas atual:")
        for orig, mapped in self.note_map.items():
            self.log_message(f"  Pad {orig} -> Nota {mapped}")
        ...
        # Iniciar nova thread
        self.translator_thread = MidiTranslatorThread(serial_port, baud_rate, midi_port, self.note_map)
